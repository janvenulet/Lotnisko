...
(
    ...
    email VARCHAR2(40) UNIQUE CHECK ( email IS NULL OR email LIKE '%@%.%') ,
...
(
    ...
    tryb_dostepu NUMBER(1) DEFAULT 0 NOT NULL CHECK ( tryb_dostepu IN (0,1,2,3)),
...
(
    ...
    dataa DATE NOT NULL,
    bramka NUMBER(6,0) CHECK ( bramka IS NULL OR bramka IN ( 1, 2, 3, 4, 5, 7)) ,
    status CHAR(1) CHECK (status IN ('z', 'o' , 'c', 'w', NULL ) ),
...
(
    ...
    id_biletu NUMBER(6,0) UNIQUE NOT NULL REFERENCES BILETY(id_biletu)
);
